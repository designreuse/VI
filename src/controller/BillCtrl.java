...
public class BillCtrl {
	...
	public static JSONObject getBillsByStudent(JSONObject inputJson) {
		JSONObject returnJson = new JSONObject();
		try {
			Student student = (Student) inputJson.get(Key.STUDENT);
			ArrayList<Bill> bills = BillDAO.getBillsByStudent(student);
			if (bills != null) {
				// iterate through the list of bills & add into jsonobject
				for (Bill bill : bills) {
